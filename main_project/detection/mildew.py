...
import cv2
from helpers import stretch_contrast, smoothen_image
...
def plot_detected_region(segment:np.ndarray, image:np.ndarray, savePath:str = None, method:str = "b", imageNo:int = 0) -> List :
    '''
    Utility function to generate results
    Inputs:
    segment => segmentation result
    image => original leaf image
    savePath => if None, results will not be saved.
    method => b for b* thresholding and KM for kmeans thresholding
    imageNo => name of the saved image
    
    returns:
    cache => List containing detected region image and segmented image.
